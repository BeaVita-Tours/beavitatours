...
type ReviewStatsBadgeProps = {
  /**
   * Outbound URL. Only the `compact` variant links out (to the platform
   * page); the three headline cards are deliberately static — see below.
   */
  href?: string;
  name: string;
  ota: PlatformId;
...
export function ReviewStatsBadge({
  ...
  // The headline cards are NOT links (client request): they are proof, not
  // navigation, and a click that leaves the site from the top of the reviews
  // section was costing visitors. The compact cluster below keeps its links.
  return (
    <div
      role="img"
      aria-label={a11yLabel}
      // Wide enough for the largest wordmark (GetYourGuide, 11.3:1) at its
      // 24px height, so every platform's mark can render at the same size.
      className="flex w-full flex-col items-center justify-center rounded-2xl border bg-card px-6 py-7 text-center sm:w-64"
    >
