...
Questa guida ti spiega, passo dopo passo, come scrivere e pubblicare gli articoli
del blog del tuo sito. Tutto avviene nel pannello "Sanity Studio" del tuo sito: un'area
riservata in cui puoi scrivere, modificare e pubblicare i post.
...
Non serve alcuna competenza tecnica: segui le istruzioni nell'ordine e tutto
funzionerà. I nomi dei campi e dei pulsanti che vedi sullo schermo sono scritti
in **grassetto** esattamente come compaiono, così puoi riconoscerli al volo.

---

## 1. Come aprire lo Studio

> Ecco il link per accedere rapidamente allo Studio nella versione demo: https://beavitatours-git-redesign-alexciaos-projects.vercel.app/studio

1. Apri il browser e vai all'indirizzo del tuo
   sito.
...
> **`/studio`** e accedere con Google per entrare nello Studio. Se usi un
> account Google con un'email diversa, non verrai riconosciuto.

Una volta dentro, vedrai un menu sulla **sinistra** con tre voci:
...
1. Nel menu a sinistra clicca **Posts**.
2. Clicca il pulsante **Create new** in alto.
   - Puoi partire da una pagina **vuota**, oppure
   - clicca su **"Post — starter"** (appare nella lista dei tipi di documento):
...
`a-day-in-the-dolomites`. **Si genera da solo** dal titolo: nella maggior
parte dei casi **non devi toccarlo**. Lascialo come sta.

> Nel caso non si generasse in automatico, basta cliccare il pulsante **Generate** accanto al campo.
...
barra sopra il testo. Puoi aggiungere:

- **Paragrafi** — il testo normale, si scrive direttamente.
...
- **Bold** — per **scrivere in grassetto**: seleziona il testo e clicca
  **B**. (A volte compare come icona "B").
- **Italic** — per il _corsivo_: seleziona il testo e clicca **I**.
- **Code** — per evidenziare una parola o una riga di codice.
- **External link** — per inserire un collegamento a un altro sito web.
  Dopo aver cliccato, ti chiederà l'**URL** (l'indirizzo, es.
...
campo è obbligatorio.** Attenzione: se metti una data nel futuro,
l'articolo resterà nascosto fino a quel momento. Per pubblicare subito,
imposta la data e l'ora di adesso (o leggermente prima).
...
- **Save** — salva quello che hai scritto senza renderlo visibile sul sito
  (resta una "bozza"). Puoi salvare quando vuoi, anche a metà lavoro.
- **Publish** — pubblica l'articolo, cioè lo rende visibile a tutti sul
  blog. Fallo solo quando sei sicuro del contenuto.

> **Regola d'oro**: per un articolo nuovo, prima compila tutto, poi clicca
...
1. Nel menu a sinistra clicca **Posts**.
2. Cerca l'articolo nell'elenco e cliccaci sopra per aprirlo.
3. Modifica i campi che vuoi (testo, immagini, categorie, ecc.).
4. Clicca **Publish** per rendere attive le modifiche sul sito.

> Se clicchi **Save** invece di **Publish**, le modifiche restano salvate ma
> **non** si vedono ancora sul sito finché non premi **Publish**.
...
2. Clicca il pulsante **Unpublish**.
...
1. Apri l'articolo da **Posts**.
2. Clicca il menu con i tre puntini (**⋯**) in alto.
3. Scegli **Delete document**.
4. Conferma quando ti viene chiesto.
...
1. Nel menu a sinistra clicca **Authors**.
2. Clicca **Create new**.
3. Compila i campi:
...
1. Nel menu a sinistra clicca **Categories**.
2. Clicca **Create new**.
3. Compila i campi:
...
In rari casi, un nuovo post può metterci **fino a un giorno** ad apparire,
perché il sito si aggiorna da solo in sottofondo. Se aspetti un po' e non vedi ancora il post,
vai alla sezione "Problemi comuni" qui sotto.

---

## 9. Problemi comuni (troubleshooting)

Prima di chiamare il tuo tecnico, controlla questi punti nell'ordine:

**1. Il post non appare sul blog**

- Hai cliccato **Publish**? Un post che ha solo **Save** è una bozza e non si
  vede. Apri l'articolo e controlla: se in alto c'è il pulsante **Publish**,
  significa che non è ancora pubblicato. Cliccalo.
- La data in **Published at** è nel passato? Se è **nel futuro**, il post è
  volutamente nascosto fino a quella data. Metti una data di oggi o più
  vecchia per renderlo subito visibile.
- Il campo **Slug** è compilato? Se manca, il post potrebbe non avere un
  indirizzo. Di solito si genera da solo; se è vuoto, clicca **Generate**
  accanto al campo.
...
- Hai salvato ma non ripubblicato. Dopo ogni modifica devi cliccare
  **Publish** (non solo **Save**) perché le modifiche diventino visibili.

**3. Ho scritto l'articolo ma ho chiuso la pagina per sbaglio**
...
- Se avevi premuto **Save**, il lavoro è al sicuro: riapri lo Studio, vai su
  **Posts** e troverai la bozza. Se **non** avevi premuto Save, il lavoro
  è andato perso. Da adesso in poi salva spesso: **Save** a metà lavoro non
  fa mai male.

**4. Ho pubblicato per sbaglio un articolo**

- Nessun problema. Apri l'articolo e clicca **Unpublish**: sparirà subito
